...
// upsertChallengeStageRecord creates or updates a record in the "learn" collection.
// It uses email + learnName as the unique key.
func upsertChallengeStageRecord(email, learnId, learnName string) error {
	fmt.Printf("[DEBUG] Upserting challenge record for Email: %s, LearnName: %s, LearnId: %s\n", email, learnName, learnId)
	...
	// ✅ CORRECT FIELD NAMES: LearnName (capital C) and email
	filter := fmt.Sprintf(`email="%s" && LearnName="%s"`,
		strings.ReplaceAll(email, `"`, `\"`),
		strings.ReplaceAll(learnName, `"`, `\"`),
	)

	// ✅ IMPROVED DEBUGGING: Show raw filter before escaping
	fmt.Printf("[DEBUG] Raw filter: %s\n", filter)
	escapedFilter := url.QueryEscape(filter)
	apiURL := fmt.Sprintf("%s/api/collections/learn/records?filter=%s&perPage=1", pbBaseURL, escapedFilter)
	...
	payload := map[string]any{
		"email":     email,
		"LearnId":   learnId,   // 👈 Capital 'C'
		"LearnName": learnName, // 👈 Capital 'C'
	}
	...
	method, requestURL := "POST", fmt.Sprintf("%s/api/collections/learn/records", pbBaseURL)
	if len(result.Items) > 0 {
		method = "PATCH"
		requestURL = fmt.Sprintf("%s/api/collections/learn/records/%s", pbBaseURL, result.Items[0].Id)
...
func TestHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Read learnId and learnName from form
	learnId := r.FormValue("learnId") // e.g., "1", "ch01", etc.
	learnName := r.FormValue("learnName")
	...
	if result.Success && result.Status == "Accepted" {
		...
		if err := upsertChallengeStageRecord(authData.Email, learnId, learnName); err != nil {
			fmt.Printf("[pb] UPSERT FAILED: email=%s learnName=%s err=%v\n", authData.Email, learnName, err)
		} else {
			fmt.Printf("[pb] UPSERT OK: email=%s learnName=%s\n", authData.Email, learnName)
		}
	}
